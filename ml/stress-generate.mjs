...
for(const family of families.filter(f=>f.split==='test'))for(const language of ['ja','zh']){
 ...
 await set();const bi=await inspect(page);const before=await page.screenshot({fullPage:true,clip:{x:0,y:0,width:900,height:await page.evaluate(()=>Math.max(innerHeight,document.documentElement.scrollHeight))},animations:'disabled',scale:'css'});const beforePath=`${out}/${family.id}-${language}-before.png`;await fs.writeFile(beforePath,before);const bp=PNG.sync.read(before);
 for(const operation of operations){await set();await mutate(page,mutationScript(operation,variant));const ai=await inspect(page);const after=await page.screenshot({fullPage:true,clip:{x:0,y:0,width:900,height:await page.evaluate(()=>Math.max(innerHeight,document.documentElement.scrollHeight))},animations:'disabled',scale:'css'});const id=`${family.id}-${language}-${operation}`,afterPath=`${out}/${id}-after.png`;await fs.writeFile(afterPath,after);const ap=PNG.sync.read(after);let changedPixels=0;for(let i=0;i<Math.max(bp.data.length,ap.data.length);i+=4){let d=0;for(let c=0;c<3;c++)d+=Math.abs((bp.data[i+c]??255)-(ap.data[i+c]??255));if(d>12)changedPixels++;}
