...
try { sourceSha = execFileSync('git', ['rev-parse', 'HEAD'], { encoding: 'utf8' }).trim(); } catch { /* Archives have no Git metadata. */ }
...
writeFileSync('dist/version.json', JSON.stringify({
  schemaVersion: '1.0', sourceSha, version: manifest.version,
