...
export function Viewer({run, selected, onSelect}: {run: Run; selected?: Candidate; onSelect:(id:string)=>void}) {
  ...
  useEffect(()=>{
    if(mode!=='difference'||!diff.current)return;
    setDifferenceStatus('Rendering exact pixel difference…');
    const worker=new Worker(new URL('./difference.worker.ts',import.meta.url),{type:'module'});
    worker.onerror=e=>setDifferenceStatus(e.message||'Difference rendering failed.');
    worker.onmessage=e=>{if(e.data.type==='error'){setDifferenceStatus(e.data.error);worker.terminate();return;}if(!diff.current)return;const {width,height,data}=e.data;const canvas=diff.current;canvas.width=width;canvas.height=height;canvas.getContext('2d')!.putImageData(new ImageData(new Uint8ClampedArray(data),width,height),0,0);setDifferenceStatus('');worker.terminate();};
    worker.postMessage({before:new URL(run.images?.before||run.beforeUrl,location.href).href,after:new URL(run.images?.after||run.afterUrl,location.href).href,masks:run.masks});
  ...
  return <section className="viewer" ref={frame} aria-label="Screenshot comparison">
    ...
    {mode==='difference'&&<div className="difference-legend">{differenceStatus||'Red: changed pixels · Magenta: height-only area · Gray / hatch: excluded mask'}</div>}
